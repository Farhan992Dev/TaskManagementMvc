...
namespace TaskManagementMvc.Controllers
{
    ...
    public class TasksController : Controller
    {
        ...
        private async Task<ApplicationUser> GetCurrentUserOrThrow()
        {
            ...
            {
                throw new InvalidOperationException("User must be assigned to a company");
            }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                // Non-admin users can only see tasks from projects they have explicit access to
        ...
        public async Task<IActionResult> Create(int? projectId)
        {
            ...
            // Auto-detect project from board URL if not provided
            if (projectId == null)
            {
                try
                {
        ...
        public async Task<IActionResult> Create(TaskFormViewModel vm, IFormFileCollection files, string? initialComment)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    // Company selection removed: company is inferred from selected project or current user
                    ...
                    // Clean up any null values that shouldn't be null
                    if (task.PerformerId != null)
                    {
                        var performer = await _userManager.FindByIdAsync(task.PerformerId.Value.ToString());
                        if (performer == null)
                        {
    ...
    public async Task<IActionResult> DeleteConfirmed(int id)
        ...
        [ValidateAntiForgeryToken]
        [Authorize(Policy = Permissions.CreateTasks)]
        public async Task<IActionResult> QuickCreate(string title, TaskStatus status, int? projectId, int? performerId, double? hours)
        {
            ...
            // Auto-assign project when creating from a board URL if not provided
            if (projectId == null)
            {
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(referer))
                    {
                        var uri = new Uri(referer);
                        var segments = uri.AbsolutePath.Split('/', StringSplitOptions.RemoveEmptyEntries);
                        // Expecting pattern: Tasks/Board/{code}
                        var boardIdx = Array.FindIndex(segments, s => string.Equals(s, "Board", StringComparison.OrdinalIgnoreCase));
                        if (boardIdx > 0 && boardIdx + 1 < segments.Length)
                        {
                            var code = segments[boardIdx + 1];
            ...
            // Validate access to project for non-admin users
            if (!User.IsInRole(Roles.SystemAdmin) && projectId != null)
            {
                var proj = await _context.Projects.FindAsync(projectId.Value);
                if (proj == null)
                {
                    await this.NotifyValidationErrorAsync("پروژه انتخاب‌شده یافت نشد.");
                    return BadRequest();
                }
                if (user?.CompanyId == null || proj.CompanyId != user.CompanyId)
                {
                    await this.NotifyPermissionErrorAsync("شما فقط می‌توانید در پروژه‌های شرکت خود تسک ایجاد کنید.");
                    return Forbid();
                }
                // Validate performer belongs to same company (when provided)
                if (performerId.HasValue)
                {
                    var perf = await _context.Users.FindAsync(performerId.Value);
                    if (perf == null || perf.CompanyId != proj.CompanyId)
                    {
                        await this.NotifyPermissionErrorAsync("انتخاب انجام‌دهنده معتبر نیست.");
                        return BadRequest();
                    }
                }
            }
            ...
            if (performerId.HasValue)
            {
                task.PerformerId = performerId.Value;
            }
        ...
        public async Task<IActionResult> SetHours(int id, double hours)
        {
            ...
            else
            {
                taskQuery = _context.Tasks
                    .Include(t => t.Project!).ThenInclude(p => p.Company)
                    .Where(t => t.Project != null && t.Project.CompanyId == user.CompanyId);
            }
        ...
        // POST: Tasks/SetPerformer
        [HttpPost]
        [ValidateAntiForgeryToken]
        [Authorize(Policy = Permissions.EditTasks)]
        public async Task<IActionResult> SetPerformer(int id, int? performerId)
        {
            var user = await GetCurrentUserOrThrow();
            IQueryable<TaskItem> taskQuery;
            if (User.IsInRole(Roles.SystemAdmin))
            ...
            {
                var perf = await _context.Users.FindAsync(performerId.Value);
                if (perf == null || (task.Project != null && perf.CompanyId != task.Project.CompanyId))
                {
                    await this.NotifyPermissionErrorAsync("انتخاب انجام‌دهنده معتبر نیست.");
                    return BadRequest();
                }
            ...
            var old = task.PerformerId;
            task.PerformerId = performerId;
            task.UpdatedAt = DateTime.Now;
            task.UpdatedById = user?.Id;
            await LogTaskChange(id, "PerformerId", old?.ToString(), performerId?.ToString(), user?.Id);
            _context.Update(task);
            await _context.SaveChangesAsync();
            return Json(new { success = true });
        }
        ...
        public async Task<IActionResult> Board(string? code)
        {
            ...
            if (!User.IsInRole(Roles.SystemAdmin) && project.CompanyId != user.CompanyId)
            {
                return Forbid();
            }
