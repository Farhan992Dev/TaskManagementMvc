...
window.SignalRNotificationHelpers = {
    ...
    formatDate(dateString) {
        ...
        const diffCompletedEstimateHours = Math.floor(diffMins / 60);
        const diffDays = Math.floor(diffCompletedEstimateHours / 24);

        if (diffMins < 1) return 'همین الان';
        if (diffMins < 60) return `${diffMins} دقیقه پیش`;
        if (diffCompletedEstimateHours < 24) return `${diffCompletedEstimateHours} ساعت پیش`;
